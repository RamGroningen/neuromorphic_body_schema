...
class FingerController:
    ...
    def __init__(self) -> None:
        ...
        self._force_magnitude = 0.5 * FINGER_FORCE_MAX
    ...
    def set_force_magnitude(self, force_magnitude: float) -> None:
        ...
        with self._lock:
            self._force_magnitude = float(np.clip(force_magnitude, 0.0, FINGER_FORCE_MAX))

    def set_cartesian_force(self, fx: float, fy: float) -> None:
        """Set the commanded XY force directly in world frame."""
        with self._lock:
            self._cartesian_force[0] = np.clip(fx, FINGER_FORCE_MIN, FINGER_FORCE_MAX)
            self._cartesian_force[1] = np.clip(fy, FINGER_FORCE_MIN, FINGER_FORCE_MAX)

    def set_joint_torques(self, proximal: float, distal: float) -> None:
        """Set finger joint torques directly (N·m).  For controller use."""
        with self._lock:
            self._joint_torques[:] = [
                np.clip(proximal, -FINGER_TORQUE_MAX, FINGER_TORQUE_MAX),
                np.clip(distal,   -FINGER_TORQUE_MAX, FINGER_TORQUE_MAX),
            ]
    ...
    def get_cartesian_force(self) -> np.ndarray:
        ...
        with self._lock:
            ...
            if norm > 0.0:
                direction /= norm
            return np.clip(direction * self._force_magnitude, FINGER_FORCE_MIN, FINGER_FORCE_MAX)
...
class FingerControlPanel:
    """Small Tk panel for manual force control of the index finger.

    Use the Fx/Fy sliders to set a signed Cartesian force directly.
    Keyboard buttons remain available as a quick override.
    """

    def __init__(self, controller: FingerController) -> None:
        self.controller = controller
        self._open = True
        self._root = tk.Toplevel()
        self._root.title("Finger Force Control")
        self._root.resizable(False, False)
        self._root.protocol("WM_DELETE_WINDOW", self.close)
        self._root.attributes("-topmost", True)
        ...
        tk.Label(
            container,
            text="Set Fx/Fy with sliders, or use I/J/K/L and arrow keys as a quick override.",
            justify="left",
            wraplength=290,
        ).grid(row=0, column=0, columnspan=3, pady=(0, 10), sticky="w")

        tk.Label(container, text="Fx").grid(row=1, column=0, sticky="w")
        self._force_x_var = tk.DoubleVar(value=0.0)
        self._force_x_label = tk.Label(container, text="0.00 N")
        self._force_x_label.grid(row=1, column=2, sticky="e")
        self._force_x_scale = tk.Scale(
            container,
            from_=FINGER_FORCE_MIN,
            to=FINGER_FORCE_MAX,
            resolution=0.05,
            orient=tk.HORIZONTAL,
            length=170,
            variable=self._force_x_var,
            command=self._on_force_change,
        )
        self._force_x_scale.grid(row=1, column=1, sticky="ew", padx=4)

        tk.Label(container, text="Fy").grid(row=2, column=0, sticky="w")
        self._force_y_var = tk.DoubleVar(value=0.0)
        self._force_y_label = tk.Label(container, text="0.00 N")
        self._force_y_label.grid(row=2, column=2, sticky="e")
        self._force_y_scale = tk.Scale(
            container,
            from_=FINGER_FORCE_MIN,
            to=FINGER_FORCE_MAX,
            resolution=0.05,
            orient=tk.HORIZONTAL,
            length=170,
            variable=self._force_y_var,
            command=self._on_force_change,
        )
        self._force_y_scale.grid(row=2, column=1, sticky="ew", padx=4)

        self._make_button(container, "Forward (I)", "forward", 3, 1)
        self._make_button(container, "Left (J)",    "left",    4, 0)
        self._make_button(container, "Stop",        None,      4, 1, stop_button=True)
        self._make_button(container, "Right (L)",   "right",   4, 2)
        self._make_button(container, "Back (K)",    "back",    5, 1)

        tk.Button(
            container,
            text="Zero Force",
            command=self._zero_force,
            width=16,
        ).grid(row=6, column=0, columnspan=3, padx=4, pady=(4, 2), sticky="ew")

        self._root.bind("<KeyPress>",   self._on_key_press)
        self._root.bind("<KeyRelease>", self._on_key_release)
        self._on_force_change(None)
        self._root.update_idletasks()

    def _on_force_change(self, _value) -> None:
        fx = float(self._force_x_var.get())
        fy = float(self._force_y_var.get())
        self.controller.set_cartesian_force(fx, fy)
        self._force_x_label.configure(text=f"{fx:.2f} N")
        self._force_y_label.configure(text=f"{fy:.2f} N")

    def _zero_force(self) -> None:
        self._force_x_var.set(0.0)
        self._force_y_var.set(0.0)
        self.controller.stop()
        self.controller.set_cartesian_force(0.0, 0.0)
    ...
    def _make_button(
        ...
        button = tk.Button(parent, text=label, width=12 if not stop_button else 8)
        button.grid(row=row, column=column, padx=4, pady=4, sticky="nsew")
        if stop_button:
            button.configure(command=self.controller.stop)
            return
        assert action is not None
        button.bind("<ButtonPress-1>",   lambda _e, a=action: self.controller.press(a))
        button.bind("<ButtonRelease-1>", lambda _e, a=action: self.controller.release(a))

    def _on_key_press(self, event: tk.Event) -> None:
        action = FingerController.key_to_action(str(event.keysym))
        if action is not None:
            self.controller.press(action)
...
class SimulationControlPanel:
    ...
    def __init__(
        ...
        self._ball_speed_var = tk.DoubleVar(value=BALL_SPEED_DEFAULT)
        self._ball_speed_label = tk.Label(ball_frame, text=f"{BALL_SPEED_DEFAULT:.3f} m/s")
        ...
        self._finger_force_var = tk.DoubleVar(value=0.5 * FINGER_FORCE_MAX)
        self._finger_force_label = tk.Label(finger_frame, text=f"{0.5 * FINGER_FORCE_MAX:.2f} N")
        ...
        self._on_ball_speed_change(f"{BALL_SPEED_DEFAULT}")
    ...
    def _on_key_press(self, event: tk.Event) -> None:
        ball_action = BallMotionController.key_to_action(str(event.keysym))
